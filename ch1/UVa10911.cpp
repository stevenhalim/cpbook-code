...
double dp(int mask) {                            // DP state = mask
  ...
  int original_mask = mask;                      // copy to a temp variable
  ans = 1e9;                                     // init with a large value
  int two_pow_p1 = LSOne(mask);                  // speedup
  int p1 = __builtin_ctz(two_pow_p1);            // p1 is first on bit
  mask -= two_pow_p1;                            // turn off bit p1
  while (mask) {
    int two_pow_p2 = LSOne(mask);                // then, try to match p1
    int p2 = __builtin_ctz(two_pow_p2);          // with another on bit p2
    ans = min(ans, dist[p1][p2] + dp(original_mask^two_pow_p1^two_pow_p2));
    mask -= two_pow_p2;                          // turn off bit p2
  }
