...
vector<vii> AdjList;         // res and AdjList contain the same flow graph
vi p;
  ...
  */
  ...
  for (int i = 0; i < V; i++) {
    ...
    for (int j = 0; j < k; j++) {
      ...
      res[i][vertex] = weight;
      AdjList[i].push_back(ii(vertex, 1));           // to record structure
      AdjList[vertex].push_back(ii(i, 1));   // do not forget the back edge
    }
  }
  ...
  while (1) {                       // an O(VE^2) Edmonds Karp's algorithm
    ...
    while (!q.empty()) {
      ...
      for (auto v : AdjList[u])     // use AdjList for neighbor enumeration
