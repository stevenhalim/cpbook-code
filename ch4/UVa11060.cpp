...
int main() {
  ...
  while (1) {
    ...
    while (!pq.empty()) {     // classical toposort algorithm: Kahn's algorithm
      int u = pq.top(); pq.pop();
      // process u
      for (auto &v : AL[u]) {
        in_degree[v]--;        // virtually 'remove' this outgoing edge of u->v
        if (in_degree[v] == 0)              // now v becomes the next candidate
          pq.push(v);                       // the smallest id will go to front
      }
    }
